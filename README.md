...
├─────────────────────────────────────────────────────────────┤
│  Layer 3.5: Themis Agent Ecosystem                          │
│  Themis Official Agent · Third-party agent deployment ·     │
│  Strategy Skills marketplace · Shared reputation ledger     │
├─────────────────────────────────────────────────────────────┤
...
## Themis Agent Ecosystem

**Status: Coming in v0.5 🚧**

Beyond providing verdict signals as raw data, Themis is building a two-sided Agent ecosystem — a platform where both Themis-native agents and third-party agents coexist, interact, and create value.

### Track A — Themis Official Agent

Themis will release its own first-party AI trading agent: **Themis Agent**.

Themis Agent is not a simple signal consumer. It is a fully autonomous execution layer built on Themis verdicts:

- **Verdict-driven execution**: Themis Agent reads live verdict signals and executes trades based on the output — direction, entry, targets, stop loss — without human intervention
- **Themis feature proxy**: Users can delegate Themis functions to the agent — running verdicts, monitoring regimes, setting alert triggers, verifying outcomes — all handled autonomously
- **Risk guardrails built-in**: drawdown caps, per-trade limits, daily loss limits, token allowlists, and slippage protection are enforced at the agent level
- **Self-custody**: signing authority stays with the user throughout the execution loop — Themis Agent never holds keys
- **On-chain reputation**: every trade executed by Themis Agent is linked to the underlying verdict, contributing to the on-chain accuracy record

Themis Agent represents the fully automated end state of the verdict production pipeline: from live CMC data → verdict → execution → on-chain record, with no human in the loop.

### Track B — Third-Party Agent Deployment

Themis is also designed as an **open platform** for other agents to deploy and operate within the Themis ecosystem.

Any developer or team can deploy their own agent on Themis and offer:

- **Strategy Skills**: custom market analysis methodologies, sector-specific verdict frameworks, or alternative evidence systems
- **Advisory services**: structured trading guidance, portfolio regime monitoring, risk assessment reports
- **Value-added integrations**: agents that combine Themis verdicts with external data sources (social sentiment, on-chain flow, derivatives positioning) to produce enhanced signals

Third-party agents on Themis benefit from:
- Access to Themis's live verdict data stream as a base signal layer
- Exposure to the Themis user base and agent subscriber network
- On-chain reputation tracking — their accuracy record is publicly verifiable, same as Themis
- Revenue sharing from subscriptions to their strategies and services

### The Two-Track Ecosystem
...
│                                                                 │
│  Track A: Themis Official Agent                                 │
│  ├── Reads live verdict signals                                 │
│  ├── Executes trades autonomously                               │
│  ├── Proxies Themis features for users                          │
│  └── Contributes to on-chain accuracy record                    │
│                                                                 │
│  Track B: Third-Party Agent Deployment                          │
│  ├── Deploy custom Strategy Skills                              │
│  ├── Offer advisory & value-added services                      │
│  ├── Access Themis verdict stream as base layer                 │
│  └── Build reputation on shared on-chain ledger                 │
│                                                                 │
│  Shared Infrastructure                                          │
│  └── Verdict stream · Reputation system · On-chain settlement   │
└─────────────────────────────────────────────────────────────────┘
```

This two-track design transforms Themis from a single product into a **verdict platform** — the infrastructure layer that both first-party and third-party agents build on top of.
...
| v0.5 | Themis Agent + third-party agent platform | 🚧 Upcoming |
...
**Layer 3.5 — Agent Ecosystem**
The two-track agent platform opens additional revenue streams:
- Themis Agent subscription: users pay to have Themis Agent execute on their behalf
- Third-party agent commission: Themis takes a platform fee on subscriptions to third-party strategies deployed on the ecosystem
- Strategy Skill marketplace: premium Skills listed by third-party agents generate recurring revenue for both the creator and the platform
